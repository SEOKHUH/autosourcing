...
```
┌─ UI (새 탭 ES 모듈 앱) ──────────────┐   사람이 각 단계 검수·수정
│   Step 1 정보검수 → 2 이미지편집       │
│   → 3 결과확인 → 4 등록                │
└───────────┬──────────────────────────┘
            │ 메시지
┌───────────┴─ Background (service worker) ─┐  1688 fetch·파싱, 번역,
│   window.context 파싱 / Gemini 번역        │  이미지 다운로드, 탭 관리
│   / 이미지 다운로드(WebP→JPEG)             │
└───────────┬───────────────────────────────┘
            │
┌───────────┴─ Content scripts ─────────────┐  페이지 위에서 직접 동작
│   쿠팡 오버레이(월판매량·카테고리ID)        │
│   1688 배너(소싱 후보 연결)                 │
│   서플라이어 허브 자동입력(executeScript)    │
└────────────────────────────────────────────┘
```
...
```
쿠팡에서 카테고리 탐색 → 월 판매량으로 수요 검증 → "소싱 후보"에 추가
   → (aliprice로 1688 공급처 역이미지검색)  ← 사람
   → 1688 상품에 소싱 후보 연결 → 크롤링
   → Step 1~4 (검수·이미지편집·확인·등록초안)  ← 사람 검수
   → 서플라이어 허브에서 최종 등록  ← 사람
```
